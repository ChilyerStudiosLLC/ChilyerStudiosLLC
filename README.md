<!--
  This README appears on the ChilyerStudiosLLC GitHub profile page.
  To activate it, this file must live in a public repo at:
    github.com/ChilyerStudiosLLC/ChilyerStudiosLLC
  i.e. a repo whose name exactly matches the username.

  The picture/source tags below auto-swap the logo for light vs dark theme.
  They reference files in the chilyer-brand repo, so logo updates there
  flow through to this README without edits here.
-->

<div align="center">

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/ChilyerStudiosLLC/chilyer-brand/main/png/lockup-horizontal-inverse-240h.png">
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/ChilyerStudiosLLC/chilyer-brand/main/png/lockup-horizontal-deep-frost-240h.png">
  <img alt="Chilyer Studios" src="https://raw.githubusercontent.com/ChilyerStudiosLLC/chilyer-brand/main/png/lockup-horizontal-deep-frost-240h.png" height="120">
</picture>

<br/><br/>

**Independent game studio. Cold mechanics, deliberate craft.**

[![X](https://img.shields.io/badge/X-@ChilyerStudios-1A2740?style=flat&logo=x&logoColor=3CDFFF)](https://x.com/ChilyerStudios)
[![Brand](https://img.shields.io/badge/Brand-Assets-1A2740?style=flat&logoColor=3CDFFF)](https://github.com/ChilyerStudiosLLC/chilyer-brand)

</div>

---

### About

I'm Chris, the developer behind Chilyer Studios — a one-person indie studio based in Tennessee building games in **Godot 4**, with a focus on 3D character work, custom assets, and tight, deliberate game feel.

### Stack

`Godot 4` &nbsp;·&nbsp; `Blender` &nbsp;·&nbsp; `GDScript` &nbsp;·&nbsp; `JavaScript` &nbsp;·&nbsp; `Claude`

### Reach me

- **X:** [@ChilyerStudios](https://x.com/ChilyerStudios) — work-in-progress posts, screenshots, dev notes
- **Brand assets:** [chilyer-brand](https://github.com/ChilyerStudiosLLC/chilyer-brand) — logos, colors, usage guidelines

### Open source

- **[blender-character-pipeline](https://github.com/ChilyerStudiosLLC/blender-character-pipeline)** — Reproducible workflow for game-ready 3D characters in Blender. Shared base mesh, body-topology costumes, stop-and-validate stage gates.

<br/>

<div align="center">
<sub>Building cold. Shipping deliberate.</sub>
</div>
